...
function ping() {
   $.getJSON(url, function(r) {
      if(r.error){  //If the Server is Offline
        ...
        $('#rest').html("<b style='margin-left:40%; color: red'>Server Offline</b> <button style='margin-left:34%' id='myBtn' class='my-button' onclick='postToServer();myPopupFunction();myCountDown()'>Turn Server ON<span class='popuptext' id='myPopup'></span></button>");
        return false;
      }
      ...
      var now = new Date();
      now = now.getHours();
     // if(r.players.online == "0" && (now >= "22" || now <= "10")) { idle(); }  //Turn Off on idle
     // if(now >= "10") { postToServer(); }  //Turn ON at 10am
      document.getElementById("buttonNotify").style.visibility="visible";
